...
const navItems = [
  { href: '/portal/agency/dashboard', label: 'OVERVIEW', icon: 'grid', exact: true },
  { href: '/portal/agency/dashboard/clients', label: 'CLIENTS', icon: 'users' },
  { href: '/portal/agency/dashboard/audits', label: 'AUDITS', icon: 'search' },
  { href: '/portal/agency/dashboard/airank', label: 'AIRANK ENGINE', icon: 'zap' },
  { href: '/portal/agency/dashboard/agent', label: 'AI AGENT', icon: 'message-square' },
  { href: '/portal/agency/dashboard/reports', label: 'REPORTS', icon: 'bar-chart' },
  { href: '/portal/agency/dashboard/settings', label: 'SETTINGS', icon: 'settings' },
]
...
function Icon({ name }: { name: string }) {
  const icons: Record<string, React.ReactNode> = {
    ...
    'message-square': (<svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>),
  ...
  const sidebarContent = (
    <aside className="w-64 h-full flex flex-col" style={{background: 'linear-gradient(180deg, #592b77 0%, #2F3B40 100%)'}}>
      ...
      <div className="px-5 py-4 border-b border-white/10 flex items-center justify-between">
        <Link href="/portal/agency/dashboard" className="flex items-center gap-2" onClick={onClose}>
          <Image

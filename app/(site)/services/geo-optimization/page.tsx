...
export default async function Page() {
  const { page, faqs: pageFaqs } = await getPageData('services-geo-optimization')
  if (!page) notFound()
  ...
  return (
    <>
      {/* Hero */}
      <section className="py-20 bg-gradient-to-br from-blue-950 to-gray-900 text-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-sm font-bold text-blue-400 uppercase tracking-widest mb-3">{hero.badge || ''}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {featureItems.map((f: any, i: number) => (
              <div key={i} className="border border-gray-200 rounded-2xl p-6 hover:shadow-lg transition-all">
                <div className="bg-blue-100 text-blue-700 w-8 h-8 rounded-lg flex items-center justify-center font-black text-sm mb-4">{i + 1}</div>
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-gradient-to-br from-blue-950 to-gray-900 rounded-2xl p-8 text-white">
